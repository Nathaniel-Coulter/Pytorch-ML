...
class SimpleLSTM(nn.Module):
    def __init__(self, input_dim=1, hidden_dim=64, num_classes=2, forget_bias=1.0):
        ...
        # Initialize forget-gate bias to encourage retention early in training.
        # LSTM biases are concatenated [i, f, g, o] along hidden dim.
        for name, p in self.lstm.named_parameters():
            if "bias" in name:
                H = hidden_dim
                with torch.no_grad():
                    p.data[H:2*H] += forget_bias
        self.fc   = nn.Linear(hidden_dim, num_classes)
    def forward(self, x):
        x = x.unsqueeze(-1)
        out, _ = self.lstm(x)
...
def train_model(model, train_loader, test_loader, epochs=10, lr=1e-3, grad_clip=1.0, device=torch.device("cpu")):
    model.to(device)
...
# Experiment sweep
# -------------------------
def run_sweep(seq_lens, seeds=3, epochs=20, n_train=20000, n_test=4000, batch=128, lr=1e-3, grad_clip=1.0, forget_bias=1.0):
...
def main():
    ...
    with open(json_path, "w") as f:
        json.dump({f"{k[0]}_L{k[1]}": v for k, v in curves.items()}, f, indent=2)
